        ...
        <div class="row mb-2">
          <div class="col-sm-12">
            <ol class="breadcrumb float-sm-left">
              ...
              <li class="breadcrumb-item"><a href="/structure/department">Kafedra</a></li>
              <li class="breadcrumb-item active">{{$department_edit->name}}</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    ...
    <section class="content">
      <div class="row">
        ...
        <div class="col-md-4">
         <div class="card card-primary card-outline rounded-0">        
             <div class="card-body">
              <form action="/structure/department/update/{{$department_edit->id}}" method="POST">
                @csrf
                <div class="form-group">
                  ...
                  <select name="faculty_id" id="faculty_id" class="faculty_select rounded-0" style = "width:100%;">
                    @foreach ($faculty_data as $faculty)
                    @if ($faculty->status == 1)
                    <option value="{{$faculty->id}}" @if ($faculty->id == $department_edit->faculty_id) selected @endif >{{$faculty->faculty_name}}</option>
                    @endif
                    @endforeach
                  </select>
                </div>
                <div class="form-group">
                  ...
                  <input type="text" class="form-control rounded-0 @error('name') border-danger @enderror" value="{{$department_edit->name}}" id="name" placeholder="" name="name">
                ...
                <div class="d-flex justify-content-end">
                  <div class="d-flex">
                    <a href="/structure/department" type="button" class="btn btn-flat btn-light border border-1 mr-1">Bekor</a>
                    <a href="/structure/department/delete/{{$department_edit->id}}" type="button" class="btn btn-flat btn-danger mr-1">O‘chirish</a>
